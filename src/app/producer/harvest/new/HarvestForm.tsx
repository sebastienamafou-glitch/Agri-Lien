...
export default function HarvestForm({ plots, producerId }: { plots: any[], producerId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // ✅ Notre fameux utilitaire DRY en action !
    await executeWithOfflineFallback(
      "Déclaration de récolte",
      
      // 1. TENTATIVE EN LIGNE (Serveur)
      async () => {
        await createHarvest(formData);
        router.push("/producer/scan"); // On redirige après succès
      },
      
      // 2. TENTATIVE HORS-LIGNE (Dexie sur le téléphone)
      async () => {
        ...
        router.push("/producer/scan"); // On redirige aussi
      }
    );
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {plots.length > 0 ? (
          <div className="relative mt-2">
            <select name="farmPlotId" required className="w-full appearance-none bg-slate-50 border-2 border-slate-100 text-slate-900 text-base font-bold rounded-2xl p-4 pr-12 focus:ring-4 focus:ring-green-500/10 focus:border-[#009A44] outline-none transition-all">
              {plots.map((plot) => (
                <option key={plot.id} value={plot.id}>{plot.name} ({plot.areaHectares} Ha)</option>
              ))}
            </select>
          </div>
        ) : (
      ...
      <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
        <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
          <Scale size={16} className="text-blue-500" /> Poids Total Estimé
        </label>
        <div className="relative mt-2">
          <input type="number" name="weight" placeholder="Ex: 450" step="0.1" min="1" required className="w-full bg-slate-50 border-2 border-slate-100 text-slate-900 text-3xl font-black rounded-2xl p-4 pl-5 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none placeholder-slate-300 transition-all"/>
